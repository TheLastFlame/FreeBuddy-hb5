...
final class HuaweiFreeBuds5Impl extends HuaweiFreeBuds5 {
  ...
  final _settingsCtrl = BehaviorSubject<HuaweiFreeBuds5Settings>();
  ...
  HuaweiFreeBuds5Impl(this._mbb, this._bluetoothDevice) {
  ...
  void _evalMbbCommand(MbbCommand cmd) {
    final lastSettings =
        _settingsCtrl.valueOrNull ?? const HuaweiFreeBuds5Settings();
    switch (cmd.args) {
      // # AncMode
  ...
  Future<void> setSettings(newSettings) async {
    final prev = _settingsCtrl.valueOrNull ?? const HuaweiFreeBuds5Settings();
